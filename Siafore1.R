##############################################################################
# Grafica Animada de los rendimientos de una Siafore                         #
...
siafore1 <- read_csv("Siafore1.csv")
...
anim2 <- siafore1 %>%
  ...
  # plot
  ggplot(aes(-rank,Value_rel, fill = Afore)) +
  geom_col(width = 0.8, position="identity") +
  coord_flip() + 
  geom_text(aes(-rank,y=0,label = Afore,hjust=0)) +       #country label
  ...
  labs(title = 'Evolución de los Rendimientos de la Siafore 1: {closest_state}',  
       subtitle  =  "Principales Afores. Fuente de Datos: CONSAR",
...
animate(anim2, 200, fps = 20,  duration = 50, width = 800, height = 600, 
        renderer = gifski_renderer("Siafore1.gif"))